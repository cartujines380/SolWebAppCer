...
app.factory('ConsSolArticuloService', ['$http', 'ngAuthSettings', function ($http, ngAuthSettings) {
    ...
    var _getArticulos = function (tipo, codigo, chkCodRef, CodRef, chkCodSap, CodSap, chkFecha, FechaDesde, FechaHasta, chkEstado, Estado, chkTipoSol, TipoSolicitud, chkLinea, LineaNegocio, Usuario, Nivel, CodProveedorCons) {

        Ruta = serviceBase + "Api/Art_Solicitud/ConsSolArticulo/?tipo=" + tipo + '&codigo=' + codigo + '&chkCodRef=' + chkCodRef +
            '&CodRef=' + CodRef + '&chkCodSap=' + chkCodSap + '&CodSap=' + CodSap + '&chkFecha=' + chkFecha + '&FechaDesde=' +
            FechaDesde + '&FechaHasta=' + FechaHasta + '&chkEstado=' + chkEstado + '&Estado=' + Estado +
            '&chkTipoSol=' + chkTipoSol + '&TipoSolicitud=' + TipoSolicitud + '&chkLinea=' + chkLinea +
            '&LineaNegocio=' + LineaNegocio + '&ArtUsuario=' + Usuario + '&ArtNivel=' + Nivel + '&CodProveedorCons=' + CodProveedorCons;
        return $http.get(Ruta).then(function (results) {
            return results;
        });
    };

    var _getSolProvLineaAdminList = function (IDNIVEL, IDLINEA, IDMODULO, IDUSUARIO) {

        return $http({
            method: 'GET',
            url: serviceBase + "Api/SolicitudProveedor/ConsultaLineaAdmin/?IDNIVEL=" + IDNIVEL + "&IDLINEA=" + IDLINEA + "&IDMODULO=" + IDMODULO + "&IDUSUARIO=" + IDUSUARIO,
        })
    };

    ConsSolArticuloServiceFactory.getArticulos = _getArticulos;
    ConsSolArticuloServiceFactory.getCatalogo = _getCatalogo;
    ConsSolArticuloServiceFactory.getSolProvLineaAdminList = _getSolProvLineaAdminList;
...
'use strict';
app.factory('AdmArticuloService', ['$http', 'ngAuthSettings', function ($http, ngAuthSettings) {

    var serviceBase = ngAuthSettings.apiServiceBaseUri;

    var AdmArticuloServiceFactory = {};
    var Ruta = '';
    ...
    var _getReporteSolicitud = function (Cab, Det, Med, CBr, Ima, Com, Cat, Alm, Iae, Ias, Iaa, Cen) {
        var SolArticulo = {
            'p_SolCabecera': Cab,
            'p_SolDetalle': Det,
            'p_SolMedida': Med,
        ...
        debugger;
        Ruta = serviceBase + 'Api/ReporteSolAdmArticulo/ReporteSolicitudArticulo/';
    ...
    var _getBajaFptArchivo = function (path, archivo) {
        ...
        Ruta = serviceBase + "Api/Art_Solicitud/BajaFptArchivo/?path=" + path + '&archivo=' + archivo + '&aux=';
        return $http.get(Ruta).then(function (results) {
            return results;
        });
    };
    ...
    var _getConsultaEAN = function (codEAN, tipoEAN) {
        debugger;
        Ruta = serviceBase + "Api/Art_Solicitud/ConsultaEAN/?codEANart=" + codEAN + "&tipoEANart=" + tipoEAN + "&isAdmin=S";
        return $http.get(Ruta).then(function (results) {
            return results;
        });
    };
    ...
    AdmArticuloServiceFactory.getConsCaracteristicas = _getConsCaracteristicas;
    AdmArticuloServiceFactory.getCatalogo = _getCatalogo;
    AdmArticuloServiceFactory.getArticulos = _getArticulos;
    AdmArticuloServiceFactory.getGrabaSolicitud = _getGrabaSolicitud;
    AdmArticuloServiceFactory.getReporteSolicitud = _getReporteSolicitud;
    AdmArticuloServiceFactory.getSecuenciaDirectorio = _getSecuenciaDirectorio;
    AdmArticuloServiceFactory.getBajaTempArchivo = _getBajaTempArchivo;
    AdmArticuloServiceFactory.getBajaFptArchivo = _getBajaFptArchivo;
    AdmArticuloServiceFactory.getConsultaEAN = _getConsultaEAN;


    return AdmArticuloServiceFactory;
