	...
	return ch.toUpperCase();
		...
		regionalOptions: { // Available regional settings, indexed by language/country code
			'': { // Default regional settings - English/US
				buttonText: '...',
				buttonStatus: 'Abrir el teclado virtual', //'Open the keypad',
				closeText: 'Cerrar', //'Close',
				closeStatus: 'Cierra el teclado virtual', //'Close the keypad',
				clearText: 'Limpiar', //'Clear',
				clearStatus: 'Limpia todo el contenido', //'Erase all the text',
				backText: 'Borrar', //'Back',
				backStatus: 'Borra el último carácter', //'Erase the previous character',
				spacebarText: '&#160;',
				spacebarStatus: 'Espacio', //'Space',
				enterText: 'Enter',
				enterStatus: 'Nueva Línea', //'Carriage return',
				tabText: '→',
				tabStatus: 'Tab Horizontal', //'Horizontal tab',
				shiftText: 'Mayús.', //'Shift',
				shiftStatus: 'Cambia la entrada de caracteres: mayúsculas/minúsculas', //'Toggle upper/lower case characters',
		...
		_generateHTML: function(inst) {
			...
			for (var i = 0; i < layout.length; i++) {
				...
				for (var j = 0; j < keys.length; j++) {
					...
					if (keyDef) {
					    html += (keyDef.action ? '<button type="button" class="btn btn-sm btn-round btn-purple ' +
					    ...
					    html += '<button type="button" class="btn btn-sm btn-round btn-purple ' + this._keyClass +
							(inst.options.useThemeRoller ? ' ui-corner-all ui-state-default' : '') +
