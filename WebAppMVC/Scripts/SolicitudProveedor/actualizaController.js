...
app.controller('BandejaSolicitudProveedorController', ['$scope', '$location', '$http', 'SolicitudProveedor', 'GeneralService', 'ngAuthSettings', '$cookies', 'localStorageService', 'authService', function ($scope, $location, $http, SolicitudProveedor, GeneralService, ngAuthSettings, $cookies, localStorageService, authService) {
    ...
    $scope.Consultar = function () {
        ...
        if ($scope.PorLinea === "2") {
            if ($scope.Ingreso.Linea == "") {
                $scope.MenjError = "Debe selecionar una Linea"
                $('#idMensajeError').modal('show');
                return;
            }
            else {
        ...
        if ($scope.txtfecdesde != "") {
            var todaydesde = new Date($scope.txtfecdesde);
            $scope.txtfecdesde = todaydesde.format("dd/mm/yyyy");

        }

        if ($scope.txtfechasta != "") {
            var today = new Date($scope.txtfechasta);
            $scope.txtfechasta = today.format("dd/mm/yyyy");
        ...
        $scope.myPromise = SolicitudProveedor.getBandejaSolicitudList($scope.txtNumero, $scope.txtfecdesde, $scope.txtfechasta, estado, "PRE", "PRE", $scope.Linea, authService.authentication.userName).then(function (results) {
...
app.controller('loginIngNuevoProvSolicitudController', ['$scope', '$location', '$http', 'SolicitudProveedor', 'GeneralService', 'ngAuthSettings', '$cookies', 'authService', 'localStorageService', function ($scope, $location, $http, SolicitudProveedor, GeneralService, ngAuthSettings, $cookies, authService, localStorageService) {
                    ...
                    if (response.data != null && response.data.length > 0) {
                        ...
                        if (response.data[0].estado != "PA") {
                            ...
                            if (response.data[0].estado == "PR") {
                                $scope.MenjError = "Su solicitud está en estado " + DesEstado;
                                $('#idMensajeInformativocali').modal('show');
                                return;
                            }

                            if (response.data[0].estado == "IP") {
                                $scope.MenjError = "Su solicitud está en estado " + DesEstado;
                                $('#idMensajeInformativocali').modal('show');
                                return;
                            }
    ...
    $scope.SolicitudActualiza = function () {
        ...
        SolicitudProveedor.getProveedorList("", "", authService.authentication.ruc, "", "").then(function (response) {
            ...
            localStorageService.set('CodProveedor', $scope.ProveedorList[0].codProveedor);
...
app.controller('BandejaSolicitudAdminProveedorController', ['$scope', '$location', '$http', 'SolicitudProveedor', 'GeneralService', 'ngAuthSettings', '$cookies', 'localStorageService', 'authService', function ($scope, $location, $http, SolicitudProveedor, GeneralService, ngAuthSettings, $cookies, localStorageService, authService) {
    $scope.SolProveedor = [];
    $scope.PorNumeroCed = 1;
    $scope.txtfecdesde = "";
    $scope.txtfechasta = "";
        ...
        if (results.data != null && results.data.length > 0) {
            ...
            for (idx = 0; idx < results.data.length; idx++) {
                ...
                if ($scope.opcion == 'APR') {
                    ...
                    if ('EN' == results.data[idx].codigo || 'AT' == results.data[idx].codigo || 'RC' == results.data[idx].codigo || 'DM' == results.data[idx].codigo) {
                        $scope.EstadoSolicitudList.push(results.data[idx]);
                    }
                }
    ...
    $scope.Consultar = function () {
        ...
        $scope.myPromise = null;
        $scope.myPromise = SolicitudProveedor.getBandejaSolicitudList($scope.txtNumero, $scope.txtfecdesde, $scope.txtfechasta, estado, "NU", $scope.opcion, $scope.Linea, authService.authentication.userName).then(function (results) {
            $scope.SolProveedor = results.data;
...
app.controller('ConsultaProveedorSController', ['$scope', '$location', '$http', 'SolicitudProveedor', 'GeneralService', 'ngAuthSettings', '$cookies', '$filter', 'FileUploader', 'localStorageService', '$timeout', '$window', function ($scope, $location, $http, SolicitudProveedor, GeneralService, ngAuthSettings, $cookies, $filter, FileUploader, localStorageService, $timeout, $window) {
    ...
    $scope.enviarobservacion = function (accion, valido, indexform) {
        ...
        if (accion != "RV" && accion != "AC" && accion != "AP" && accion != "RA" && accion != "RD") {
            ...
            $scope.SaveProveedorList(0, true, indexform);
        }
    }
            ...
            $scope.myPromise = SolicitudProveedor.getDocAdjuntoList($scope.codSapProveedor).then(function (response) {
                if (response.data != null && response.data.length > 0) {
                    ...
                    for (index = 0; index < response.data.length; index++) {
                        ...
                        $scope.p_SolDocAdjunto.permiteEliminar = false;
            ...
            if ($scope.isDisabledadjunto == true) {//edita
                ...
                for (index = 0; index < $scope.SolDocAdjunto.length; index++) {
                    if ($scope.SolDocAdjunto[index].id == $scope.p_SolDocAdjunto.id) {
                        ...
                        $scope.SolDocAdjunto[index].permiteEliminar = true;
    ...
    $scope.disableClickadjunto = function (ban, content) {
        ...
        if (content != "") {
            ...
            $scope.p_SolDocAdjunto.permiteEliminar = true;
    ...
    $scope.SaveProveedorList = function (bandera, valido, d, pestana, conMensaje) {
        ...
        if (pestana == "D" || pestana == "") {
            ...
            if ($scope.Ingreso.Region != '' && $scope.Ingreso.Region != null) {
                ...
                $scope.MenjError = "Provincia de la pestaña Datos del proveedor es requerida";
                $('#idMensajeError').modal('show');
                return;
            }
            ...
            if ($scope.Ingreso.LineaNegocio != '' && $scope.Ingreso.LineaNegocio !== undefined && $scope.Ingreso.LineaNegocio != null) {
                $scope.p_SolProveedor.LineaNegocio = $scope.Ingreso.LineaNegocio.codigo;
            }
            else {
                $scope.MenjError = "Seleccione al menos una Línea de Negocio de la pestaña Datos del proveedor.";
                $('#idMensajeError').modal('show');
                return;
            }
            ...
            if ($scope.Ingreso.GrupoCompra != undefined && $scope.Ingreso.GrupoCompra != null) {
                if ($scope.Ingreso.GrupoCompra != '') {
                    $scope.p_SolProveedor.GrupoCompra = $scope.Ingreso.GrupoCompra.codigo;
                }
            }
            else {
                $scope.MenjError = "Grupo de Compras de la pestaña Datos del proveedor es requerido";
                $('#idMensajeError').modal('show');
                return;
            }
        }
        ...
        if (pestana == "CO" || pestana == "") {
            if ($scope.p_SolProveedor.CodGrupoProveedor != '' && $scope.p_SolProveedor.CodGrupoProveedor != null) {
                ...
                if ($scope.p_SolProveedor.CodGrupoProveedor.length < 10) {
                    $scope.MenjError = "RUC Proveedor Padre es Inconrecto de la pestaña Compras - DM.";
                    $('#idMensajeError').modal('show');
                    return;
                }
        ...
        $scope.myPromise = SolicitudProveedor.getPostProveedorList($scope.SolProveedor, $scope.SolProvContacto, $scope.SolProvBanco, $scope.SolProvDireccion, $scope.SolDocAdjunto, $scope.ViaPago, $scope.SolRamo, $scope.SolZona, $scope.SolProvHistEstado, listaLinNegocio).then(function (response) {
            ...
            if (conMensaje == 1) {
                for (var i = 0; i < $scope.SolDocAdjunto.length; i++) {
                    $scope.SolDocAdjunto[i].permiteEliminar = false;
                }
